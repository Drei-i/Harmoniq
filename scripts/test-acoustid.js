...
function printReport(label, report) {
  ...
  if (report.match) {
    console.log(`Match: ${report.match.matchedArtist} — ${report.match.matchedTrack}`);
    console.log(`Release: ${report.match.label}`);
  }
  console.log(`Message: ${report.licenseAlternative?.text || report.match?.recommendation}`);
}

async function run() {
  console.log('Harmoniq AcoustID integration test\n');

  const knownResults = await lookupFingerprint(641, DOC_FINGERPRINT);
  printReport('Known fingerprint (AcoustID docs example — expect Match Found)', buildReport({
    filename: 'm83-sample.ogg',
    fileSize: 'N/A',
    format: 'OGG',
  ...
  const obscurePath = path.join(__dirname, '..', 'uploads', 'obscure-track.mp3');
  try {
    const obscureReport = await verifyAudioFile(obscurePath, {
      filename: 'obscure-track.mp3',
      fileSize: 'N/A',
      format: 'MP3'
    });
    printReport('Obscure track (SoundHelix — expect No Match Found)', obscureReport);
  } catch (error) {
    console.log('\n=== Obscure track ===');
    console.log(`Skipped: ${error.message}`);
    console.log('Run: npm run test:acoustid after placing an MP3 at uploads/obscure-track.mp3');
  }
}
